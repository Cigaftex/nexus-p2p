...
class MainFlutterWindow: NSWindow {
  private var transportAdapter: BonjourTransportAdapter?
  ...
  override func awakeFromNib() {
    ...
    transportAdapter = BonjourTransportAdapter(channel: channel)
...
private final class BonjourTransportAdapter: NSObject, NetServiceBrowserDelegate, NetServiceDelegate, CBCentralManagerDelegate, CBPeripheralManagerDelegate {
  ...
  init(channel: FlutterMethodChannel) {
    ...
    channel.setMethodCallHandler { [weak self] call, result in
      ...
      case "startBonjour":
        ...
        self.start(deviceId: deviceId, displayName: displayName, signingKey: signingKey, exchangeKey: exchangeKey, version: version, port: port)
        result(nil)
      case "stopBonjour":
        self.stop()
        result(nil)
  ...
  private func start(deviceId: String, displayName: String, signingKey: String, exchangeKey: String, version: Int, port: Int) {
    stop()
    ...
    let service = NetService(domain: "local.", type: "_nexus-p2p._tcp.", name: "nexus-\(deviceId.prefix(8))", port: Int32(port))
  ...
  func netServiceDidResolveAddress(_ sender: NetService) {
    ...
    let txt = NetService.dictionary(fromTXTRecord: txtData).compactMapValues { String(data: $0, encoding: .utf8) }
    guard let deviceId = txt["id"], deviceId != localDeviceId,
          let displayName = txt["name"], let signingKey = txt["sign"], let exchangeKey = txt["exchange"],
          let host = bestHost(from: sender.addresses ?? []), sender.port > 0 else { return }
    peerIds[ObjectIdentifier(sender)] = deviceId
  ...
  private func rank(_ candidate: (family: Int32, host: String)) -> Int {
    if candidate.family == AF_INET && !candidate.host.hasPrefix("169.254.") && candidate.host != "127.0.0.1" { return 0 }
    if candidate.family == AF_INET && candidate.host != "127.0.0.1" { return 1 }
    if candidate.family == AF_INET6 && !candidate.host.hasPrefix("fe80:") && candidate.host != "::1" { return 2 }
    return 3
  }
  ...
  private func numericHost(from data: Data) -> (family: Int32, host: String)? {
    data.withUnsafeBytes { bytes in
      ...
      let status = getnameinfo(address, socklen_t(data.count), &host, socklen_t(host.count), nil, 0, NI_NUMERICHOST)
      guard status == 0 else { return nil }
      return (Int32(address.pointee.sa_family), String(cString: host))
    }
  }

  func centralManagerDidUpdateState(_ central: CBCentralManager) { startBluetooth() }
  func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) { startBluetooth() }

  private func startBluetooth() {
    guard !localDeviceId.isEmpty else { return }
    if central?.state == .poweredOn {
      central?.stopScan()
      central?.scanForPeripherals(withServices: [Self.proximityService], options: [CBCentralManagerScanOptionAllowDuplicatesKey: true])
    }
  ...
  func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String: Any], rssi RSSI: NSNumber) {
    ...
    channel.invokeMethod("bleProximity", arguments: ["deviceIdPrefix": prefix, "rssi": RSSI.intValue])
  }
}
